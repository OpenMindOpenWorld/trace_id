...
mod tests {
    ...
    fn test_trace_id_creation() {
        ...
        // 验证只包含小写十六进制字符
        assert!(id_str.chars().all(|c| c.is_ascii_hexdigit() && (c.is_ascii_digit() || c.is_ascii_lowercase())));
    ...
    fn test_from_string_validated() {
        ...
        assert_eq!(TraceId::from_string_validated("toolongtraceidentifierthatexceeds32chars"), None);
        
        // Invalid case: 包含大写字符
        assert_eq!(TraceId::from_string_validated("0AF7651916CD43DD8448EB211C80319C"), None);
        
        // Invalid case: 包含非十六进制字符
        assert_eq!(TraceId::from_string_validated("0af7651916cd43dd8448eb211c80319g"), None);
        
        // Invalid case: 全为零
        assert_eq!(TraceId::from_string_validated("00000000000000000000000000000000"), None);
    }
    ...
    fn test_trace_id_uniqueness() {
        ...
        for _ in 0..1000 {
            let trace_id = TraceId::new();
            assert!(ids.insert(trace_id.as_str().to_string()), "Generated duplicate trace ID");
        }
    }
